...
1. **因果链**：生成与试错成本下降 → 同一任务产生更多代码/分支/工具调用 → 环境、状态、权限、失败面复杂度上升 → **稀缺性从「代码」转向「可信结果」** ^[raw/articles/lencx-agent-dev-guide-20260806.md]
2. **终点重定义**：Agent 原生软件的目标不是「最大自治」，而是「让一个不完全可靠的推理者，能够在可靠系统中持续承担责任。」 ^[raw/articles/lencx-agent-dev-guide-20260806.md]
...
生产 Agent 因而需要：^[raw/articles/lencx-agent-dev-guide-20260806.md]
...
> Context 是每次采样的视野，Loop 是时间轴，Graph 是任务拓扑，Skill 是可加载的程序性模块；**Harness 持有目标**，并把它们组织成一次可恢复执行。 ^[raw/articles/lencx-agent-dev-guide-20260806.md]

### OpenAI Harness Engineering 正式定义

不再只优化模型「看到什么」，而是让工程师**设计环境、表达意图、建设反馈环路**，使 Agent 能可靠工作 ^[ref 4^][raw/articles/lencx-agent-dev-guide-20260806.md]。

### Anthropic Managed Agents 拆分

- **Session** = 追加写的事件日志
- **Harness** = 调用模型并路由工具的循环
- **Sandbox** = 执行代码与文件动作的环境 ^[ref 5^][raw/articles/lencx-agent-dev-guide-20260806.md]

### Harness 删减 vs 模型升级

Anthropic 披露：Claude Code 为 Opus 5 / Fable 5 删掉 **>80%** system prompt，coding eval 无可测量损失 ^[ref 6^][raw/articles/lencx-agent-dev-guide-20260806.md]。每条长期指令都应有退出条件；模型升级时 Harness 应删除过时脚手架。
...
Codex Goal ^[ref 7^] 把长任务责任边界落实为产品原语。`/goal` 文本既是第一条 prompt，也是完成判据：
...
至少应拆成 5 类 ^[raw/articles/lencx-agent-dev-guide-20260806.md]：
...
- **Handoff**：当前 Agent 把会话控制权转给 specialist ^[ref 9^]

**信任 handoff** 需要结构化移交：目标与范围、已验证事实、未决假设、当前 checkpoint、待确认副作用、能力与审批状态、资源租约、停止条件、结果应交还谁。**只传一段自然语言摘要，会把上下文丢失伪装成组织分工。**

### Claude Code Dynamic Workflows

Claude 动态写 orchestration script，把任务分发给数十至数百个并行 subagent，加独立尝试、反驳、review、聚合、修复循环，并持久化进度以便中断后继续 ^[ref 10^]。Bun 的 Zig→Rust 迁移 ^[ref 11^] 19 个合并后 regression 已足够说明边界：**Agent 可把机械迁移推到既有验收器覆盖的位置，测试没有表达的行为仍会逃逸。**
...
| 项目 | 责任定位 | 关键差异 |
|------|---------|---------|
| **AI SDK 7** ^[ref 13^] | 统一模型调用 + HarnessAgent + SandboxSession | 扩为"组织受控执行" |
| **Chat SDK** ^[ref 14^] | 适配 Slack/Teams/Discord/Telegram 等 11 渠道 | |
| **Streamdown** ^[ref 15^] | 流式 Markdown | 适应长输出 |
| **json-render** ^[ref 16^] | 受 catalog/schema 约束的 UI 规范 | UI 变中间表示 |
| **Vercel Sandbox** ^[ref 17^] | Firecracker microVM | 隔离与兼容环境 |
| **agent-browser** ^[ref 18^] | 浏览器 CLI/daemon，accessibility snapshot | 观察—行动—再观察压缩成低 token 机器接口 |
| **portless** ^[ref 19^] | 稳定 `.localhost` 域名、自动端口分配 | 解决 Agent 并行 worktree 寻址 |
| **ai-cli** ^[ref 20^] | 图像/视频/音频/文本 stdin/stdout + JSON metadata | Agent 偏好的执行工具形状 |
| **scriptc** ^[ref 21^] | TS → typed IR → LLVM/C → native | 静态证明 + QuickJS 动态层 + 显式拒绝 |
| **Native SDK** ^[ref 23^] | Zig engine + `.native` markup，record/replay | UI 本身可观察、可驱动、可回放 |
| **vgpu** ^[ref 24^] | Agent-first WebGPU + deterministic mock | 显式 GPU context + WGSL reflection |
| **Turborepo** ^[ref 25-30^] | Task DAG + 哈希 + 远程缓存 | "可执行、可缓存、可检查的确定性工作图" |
| **SWC** ^[ref 31-33^] | TS/JS 编译热路径 | Next.js Compiler 默认 |
...
| 项目 | 责任 |
|------|------|
| **Agents** ^[ref 45^] | Durable Objects + SQLite + lifecycle + WebSocket + Schedule + MCP + Workflows/HITL |
| **Browser Run** ^[ref 48-49^] | 原 Browser Rendering，headless Chrome + CDP + Puppeteer/Playwright |
| **Nimbus** ^[ref 53^] | `.md/.mdx` twins + llms.txt/full + JSON-LD + Agent handoff |
| **Cloudflare Skills** ^[ref 54^] | 平台操作知识 + commands + 远程 MCP |
| **Agent Readiness** ^[ref 55^] | 站点机器界面 |
| **Code Mode** ^[ref 56^] | 隔离 Worker 跑组合代码，凭据留宿主 |
...
**关键拆分**：已弃用 Sandbox SDK 的 desktop feature ^[ref 52^]，把浏览器自动化迁到 Browser Run。**代码执行环境与浏览器身份/会话宿主分开**：前者承载不受信任计算，后者承载昂贵、长寿、带凭据的 Web 能力。统一称 "sandbox" 会丢失差异。
...
开源项目与 Skills **既服务开发者分发，也会进入模型上下文、代码语料与团队规范，形成"默认选择先验"** ^[raw/articles/lencx-agent-dev-guide-20260806.md]。平台公司开放边缘 SDK/协议，在托管执行、全局状态、身份、GPU、浏览器、审计层变现。
...
| 模型公司 | 收购 | 补齐 |
|---------|------|------|
| Anthropic | **Bun** ^[\[65\]^] (2025-12) | JS/TS runtime + 包管理 + bundler + test runner |
| Anthropic | Vercept ^[\[66\]^] | Computer use 视觉 + GUI |
| Anthropic | Stainless ^[\[67\]^] | OpenAPI → SDK/CLI/MCP |
| OpenAI | **Promptfoo** ^[\[68\]^] | Agent eval、red-team、安全 |
| OpenAI | 拟收购 Astral ^[\[69\]^] | uv/Ruff/ty |
| OpenAI | 拟收购 Ona ^[\[70\]^] | 持久云环境 |
| OpenAI | Sky ^[\[71\]^] | 屏幕理解 + macOS 操作 |

**三种生态控制关系**：

1. **所有权整合** — Bun 代表
2. **影响力合作** — **OpenClaw Foundation** ^[ref 72^]：作者加入 OpenAI → 项目入 501(c)(3) → OpenAI 资助 + Claw Labs 合作。OpenAI 获得 Gateway/Harness 一手经验，保留 OpenClaw 跨模型位置
3. **中立标准治理** — MCP / AGENTS.md 入 Linux Foundation AAIF ^[ref 73^]
...
| **Claude Cowork** ^[ref 74^] | Claude Code agentic approach 复用 → 本地文件 + Slack/Drive + 浏览器 + subagent + scheduled task + 知识工作 artifact |
| **Claude Design** ^[ref 75^] | chat + canvas + direct manipulation + /design-sync 双向同步 |

**Anthropic 2026-07 披露**：Cowork **>90% 使用量来自软件开发之外**，主要集中在业务运营与内容创作 ^[ref 77^]。

### OpenAI 桌面宿主共享

2026-07 ChatGPT desktop shell 同时容纳 **Chat / Work / Codex**。内置 browser 同时服务 Work 和 Codex ^[ref 79^]。**桌面宿主与能力平面共享，但不抹平两个产品各自的任务状态**。
...
终端为 Agent 提供的共同属性 ^[raw/articles/lencx-agent-dev-guide-20260806.md]：
...
**Google 案例**：官方公告先肯定终端是优秀的 Agent 界面，再指出多 Agent/异步任务/跨界面需要统一后端。**新 Go CLI 支持后台异步工作，与 Antigravity 2.0 桌面应用共享 Agent harness**，会话可从终端导出到 GUI 继续 ^[ref 87^]。**同一 core engine 两个表面**：CLI 面向键盘/SSH/脚本，桌面端面向视觉编排。
...
`rg` 启动快、只读、跨平台、无需常驻索引；`rg --files` 枚举候选；`--json` 接入脚本化循环。**进入 Agent 回路后**，默认规则跳过 ignored/hidden/binary、不跟 symlink，**直接影响搜索可见性与结果复现** ^[ref 92^]。

### Gateway 让 Agent 常驻

把 Agent 接进 Telegram/Discord/Slack/WhatsApp。**IM 是异步任务的 interrupt、attention、delivery surface，价值远高于聊天皮肤。**

Gateway 必须处理模型之外的问题 ^[raw/articles/lencx-agent-dev-guide-20260806.md]：
...
**OpenClaw 特色**：Gateway 统一 channel/operator client/node capability/session/approval/delivery/child lifecycle。**审批做成持久、可核验的授权记录**，绑定 node/argv/plan/cwd/agent/session/channel/thread/requester device，**`allow-once` 只能消费一次** ^[ref 98^]。这比 Telegram 中一个脱离原动作的"同意"更接近 capability authorization。
...
**Anthropic 2026-04 重做 Claude Code Desktop** ^[ref 100^]：并行 session + 集成终端 + 文件编辑器 + Diff + HTML/PDF preview + 拖拽布局。**新界面让并行任务同时在途，由人坐在 orchestrator seat**。GUI 增长发生在 Agent core 上方，承担监督、证据审阅、人工接管。

**Codex ` /app`** ^[ref 101^]：CLI 端可把当前会话继续到桌面应用；桌面端放在同一监督工作台。

### Pet 管注意力

**Pet 在 vibe coding 中的真实作用**：把 Agent 状态从工作台内的主动查询，转化为周边视野中的被动感知 ^[ref 102^][raw/articles/lencx-agent-dev-guide-20260806.md]。
...
Agent 浪潮重新放大浏览器，因为浏览器把 5 种通常分散的能力放进同一宿主 ^[raw/articles/lencx-agent-dev-guide-20260806.md]：
...
1. **既有浏览器内置 AI** — Gemini in Chrome ^[ref 104^]：side panel + 多标签上下文 + Connected Apps + auto browse
2. **新浏览器围绕 AI 重新设计** — Atlassian 收购 The Browser Company ^[ref 107^] → **Dia** ^[ref 106^]：跨 tabs/history/G Suite/Slack/Notion 上下文 + memory + Skills
3. **更大的 Agent 工作台吸收浏览器能力** — OpenAI **Atlas** ^[ref 110^]：将在 2026-08-09 停止工作，浏览器能力进入 ChatGPT + Codex

**Dia 2026-06 现状** ^[ref 109^]：Dia 正把 Teamwork Graph、Morning Brief、SSO、Chromium MDM、SOC 2 与 Guard 集成带入企业；产品重心从 Arc 式消费浏览器转向企业知识图谱与 Agent action 前台。
...
- **CDP** ^[ref 111^]：检查、调试、控制 Chromium 大量内部域 → browser agent 事实控制 ABI。但 tip-of-tree 经常变化，**Playwright 明确说 `connectOverCDP` 比自己协议"显著更低保真"**；WebDriver BiDi ^[ref 112^] 截至研究日仍是 Working Draft
- **WebMCP** ^[ref 113-115^]：后端 MCP 的客户端补充。页面把表单/JS 函数注册为带自然语言描述 + schema 的工具。**复用当前页面、登录态、可见 UI、人类上下文**；可在工具完成前等待人类确认
...
**vgpu 价值** ^[\[24\]^]：原生 API 显式而低级，WGSL/layout/buffer/pipeline 容易写错；Agent 生成代码尤其需要反射、稳定错误、无 GPU mock。未来 agent-friendly GPU 抽象应提供：capability discovery + 明确资源生命周期 + 静态/运行时 schema + 可序列化命令 + deterministic simulation + 可比较 trace + 真实 GPU 与 mock 的差异报告。

### 能力越大，风险越集中

**浏览器凭据近似物**：browser profile、CDP WebSocket URL、session token、下载目录都接近凭据。

**可靠设计至少需要** ^[raw/articles/lencx-agent-dev-guide-20260806.md]：
...
OpenAI 官方将 Codex App 描述为管理并行 Agent/worktree/diff review/Skills/Automations/系统级 sandbox 的桌面 "command center"，后加 in-app browser/computer use/文件/终端等工作面 ^[ref 127^]。**Codex Desktop 验证了"浏览器能力 + 本地执行 host + 多任务桌面工作台"的架构价值。**
...
> "Rust/Zig 成为底层、TypeScript/React/Tailwind 成为上层标配" 描述的是**分工**，而非全栈语言替代。 ^[raw/articles/lencx-agent-dev-guide-20260806.md]
...
| **Vite 8** ^[ref 131^] | dev server + HMR + transform + build + 插件图；Rust 编写的 Rolldown | **`server.forwardConsole` 让浏览器 console/error 回传 dev server 终端**，检测到 coding agent 时自动启用 |
| **Tailwind CSS v4** ^[ref 132^] | first-party Vite plugin + CSS-first config + 原生 theme variables + data/state variants | design token/组件状态/源码修改在同一份可搜索文本 |
| **Base UI** ^[ref 133^] | 无样式 + 可访问 + 可组合 primitives | focus/keyboard/ARIA/popup 边缘情况 |
| **shadcn/ui** ^[ref 134-135^] | registry → 源码分发 | Agent 可直接读取/修改/diff/迁移；2026-07 Base UI 设为新项目默认 primitive |
...
TS 7.0 ^[ref 136^] (2026-07-08) 用 Go 原生重写 compiler + shared-memory multithreading → 完整构建通常 8-12 倍提升。**7.0 暂未提供原 compiler API，新程序化 API 计划在 7.1**；Vue/MDX/Astro/Svelte/Angular/自定义 transformer 可能仍需 TS 6。

### Python 守住生态，uv 重做工具链

uv ^[ref 137^] 用 Rust 实现单一、预编译的 Python package/project manager，整合 pip/pip-tools/pipx/Poetry/pyenv/virtualenv。**Python 拥有模型/数据/Notebook/科研/PyTorch 生态，Rust/C++/CUDA 承担包管理/解析/tokenizer/数据路径/内核**。

### Bun 转 Rust：迁移快了，验证贵了

PR #30412 ^[ref 138^] (2026-05-14) 合并 "Rewrite Bun in Rust"：~100 万新增行 / 2188 文件 / 6755 commits。**既有测试各平台通过，binary 缩小 3-8MB，性能持平或更快，架构基本不变，未采用 async Rust**。分支和提交历史显示大量 Claude 辅助工作。
...
**社区合并后立刻报告** safe Rust 下的潜在 UB/Miri 问题 ^[ref 139^]：**测试通过只覆盖已有验收面，Rust 完整安全收益还依赖 invariant/sanitizer/Miri/fuzz/review/灰度发布**。**未来大型重写会更多，验证成本也会比生成速度更值钱。**

### 终端是系统语言压力测试场

**窄渲染与可编程宿主**：

- **Alacritty** ^[ref 140^] — Rust + OpenGL 窄而可组合
- **kitty** ^[ref 141-142^] — C/Python/Go 分层 + JSON remote-control 协议
- **WezTerm** ^[ref 143-144^] — Rust core + Lua 配置 + multiplexer + SSH

**可嵌入内核**：

- **Ghostty** ^[ref 145^] — Zig core + 原生 UI + C ABI
- **libghostty-vt** ^[ref 146^] — 已暴露 VT parsing/screen/scrollback/input encoding/formatter 为 C/Zig API，覆盖 Wasm
- **turborepo-ghostty** ^[ref 147^] — Turborepo 解析和渲染任务输出

**终端扩成工作台**：

- **Warp** ^[ref 149-150^] — Rust + 自研 GPU UI + block model
- **Wave** ^[ref 151^] — Go backend + Electron/React + durable SSH + wsh

**Rust 已形成完整 CLI 产品语法** **[ref 152-161^]**：
...
**完整的机器操作面** ^[raw/articles/lencx-agent-dev-guide-20260806.md]：

- Skills → 何时/为何/按什么顺序做（程序性知识）
- 身份、权限、运行时能力 → 相邻层提供

**Vercel skills CLI** ^[ref 39^] 已支持 Codex / Claude Code / Cursor 等大量 Agent 的 add/use/find/update/remove；Cloudflare 也维护自己的官方 Skills ^[ref 54^]。**分发机制已形成，供应链治理仍处早期。**

### find-skills 本身也是 Skill ^[ref 169^] — 递归能力获取
...
**Agent Skills specification** ^[ref 170^] 强调渐进披露：先目录 metadata → 按需读 SKILL.md → 才加载引用和脚本。Cloudflare `.well-known/agent-skills` discovery RFC ^[ref 171^] 要求 SHA-256 + 归档安全检查 + 默认不执行脚本。**Cloudflare Agents 对 Skills 的运行时支持仍明确为 experimental：格式已快速扩散，可信执行远未成熟。**

### Vercel skills lock 设计 ^[ref 41-43^]
...
面对 Agent 任务，先回答 ^[raw/articles/lencx-agent-dev-guide-20260806.md]：
...
可机器操作的接口需 ^[raw/articles/lencx-agent-dev-guide-20260806.md]：
...
> 如果收益只是"技术上更酷"，Agent 只会更快地产生一笔更大的维护债。 ^[raw/articles/lencx-agent-dev-guide-20260806.md]
...
> Agent 能否被托付，最后取决于系统能否回答四个具体问题：**它做了什么，依据是什么，结果是否成立，失败后如何恢复。** ^[raw/articles/lencx-agent-dev-guide-20260806.md]
...
| 主题 | 文章论断 | 关联 Wiki 页 |
|------|---------|-------------|
| Harness Engineering 正式定义 | OpenAI 提出 ^[\[4\]^]；Anthropic Managed Agents 拆分 session/harness/sandbox ^[\[5\]^] | [[Harness Engineering]] |
...
- 关键官方原文：
  ...
  - Anthropic Claude 5 context engineering 复盘: https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models
  - Codex Goal: https://learn.chatgpt.com/docs/long-running-work
  - Vercel skills: https://github.com/vercel-labs/skills
  - Cloudflare agent-skills RFC: https://github.com/cloudflare/agent-skills-discovery-rfc
